...
def get_catalog():
    ...
    with db.engine.begin() as connection:
            ...
            result = connection.execute(sqlalchemy.text("SELECT sku, name, price, type FROM potions_duplicate JOIN potions_duplicate.id ON potion_ledger.potion_id GROUP BY potions_duplicate.id HAVING SUM(potion_ledger.change) > 0"))
